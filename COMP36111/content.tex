...
optimum on a graph becomes rather hard. I don't know about you, but I'm not
great at drawing 3d graphs, and visualising n-dimensional graphs quickly becomes
infeasible.
...
\begin{itemize}
  ...
  It should be quite obvious how we go from the equations in step 1 to
  the tableaux in step 2. The only stuff you need to \textit{remember} about
  ...
  \begin{center}
    \begin{tabular}{>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|
      ...
      \rowcolor{highlight}
      m_2 & 0 & 2 &\cellcolor{dhighlight}-3&0&1& 0   & 0   &-1 & -1/2\\ \hline
      m_3 & 2 & 0 &-1                      &0&0& 1   & 0   & 0 & 0/0 \\ \hline
  ...
  \item[\textbf{Step 4}] Now we do the pivot about the pivot cell:
    ...
    \begin{itemize}
      ...
      \begin{center}
        \begin{tabular}{>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|
          >{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}}
              & x & y & z & m_1 & m_2 & m_3 & f & \text{constants}\\ \hline
          m_1 & 1 & 1           & 1&1&0            &0&0& 4\\ \hline
          m_2 & 0 & \frac{-2}{3}&\cellcolor{dhighlight}1&0&\frac{-1}{3}&0&0&\frac{1}{3}\\ \hline
          m_3 & 2 & 0           &-1&0&0            &1&0& 0\\ \hline
          f   &-1 &-2           &-3&0&0            &0&1& 0\\
        \end{tabular}
      \end{center}
      ...
      $R3 = R3 + R2$, $R4 = R4 + 3(R2)$ and $R1 = R1 - R2$ to make the cell 
      below the pivot $0$.
      ...
      \begin{center}
        \begin{tabular}{>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|
          >{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}}
              & x & y & z & m_1 & m_2 & m_3 & f & \text{constants}\\ \hline
          m_1 & 1 & \frac{5}{3} & 0&0&\frac{1}{3} &0&0&\frac{11}{3}\\ \hline
          m_2 & 0 & \frac{-2}{3}& 1&0&\frac{-1}{3}&0&0&\frac{1}{3}\\ \hline
          m_3 & 2 & \frac{-2}{3}& 0&0&\frac{-1}{3}&1&0&\frac{1}{3}\\ \hline
          f   &-1 &\frac{-12}{3}& 0&0&-1           &0&1& 1\\
        \end{tabular}
      \end{center}
      ...
      \begin{center}
        \begin{tabular}{>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}|
          >{$}c<{$}|>{$}c<{$}|>{$}c<{$}|>{$}c<{$}}
              & x & y & z & m_1 & m_2 & m_3 & f & \text{constants}\\ \hline
          m_1 & 1 & \frac{5}{3} & 1&0&\frac{1}{3} &0&0&\frac{11}{3}\\ \hline
          z   & 0 & \frac{-2}{3}& 1&0&\frac{-1}{3}&0&0&\frac{1}{3}\\ \hline
          m_3 & 2 & \frac{-2}{3}& 0&0&\frac{-1}{3}&1&0&\frac{1}{3}\\ \hline
          f   &-1 &\frac{-12}{3}& 0&0&-1           &0&1& 1\\
        \end{tabular}
      \end{center}
    ...
    \[
      m_1 = \frac{11}{3}, z = \frac{1}{3}, m_3 = \frac{1}{3}, f = 1
    \]
    ...
    \begin{alignat*}{12}
      4 &=~ &0~ &+~&0~&+~&\frac{1}{3} ~&+~&\frac{11}{3}&~ &~&~ &~&~&~\\
      -1&=~ &~~ &+~&0~&-~&3\frac{1}{3}~&~~&~           &+~&0&~ &~&~&~\\
      0 &=~ &0~&~~&~~ &-~&\frac{1}{3} ~&~~&~           &~ &~&+~&0&~&~\\
      0 &=~ &-0~&-~&0~&-~&3\frac{1}{3}~&~ &~  &~ &~  &~ &~  &+&1
    \end{alignat*}

    {\color{red}Except for the third one, where I must have made a mistake...}
