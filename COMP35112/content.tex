...
al wait. It's natural when threads are used to implement data parallelism.
...
time, and other threads must wait to aquire the lock, or the lock holder
executes \texttt{wait}.
...
Deadlocks can occur in cases where code tries to aquire more than one lock.
...
impossible using the implcit locks, since there is an implicit nesting structure
