...
continous, so there are an infinate number of parameters to test. The following
algorithms try to find global optimums:

\begin{description}
  \item \textbf{Grid Search}:\\
  We overlay a grid onto the search space (in as many dimensions as we need to),
  and test each point on the grid. We can change the resolution of thegrid by
  ...
  at these points. The point that gives the best value for th eobjective
  ...
  global minimum, since it's definately a local minimum too), but it can degrade
  performance.
  ...
  \begin{description}
    \item \textbf{Metropolis algorithm}:\\
      This is an algorithm that emulates an ensemble of particles in equlibrium
      ...
      molten very slowly so that the material can reach an equlibrium at each
      temperature. If the cooling is too fast, then an equlibrium will not be
    ...
    \item \textbf{Simulated Annealing}:\\
      ...
      \textit{asymptotocally complete}.

    \item \textbf{Press' mosification}:\\
