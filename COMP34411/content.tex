...
should read up on them and implement one!}
...
In order to find the relationships between words in a sentence, we should first
...
\marginpar{If you don't like the cosine stuff, you can also use the Euclidean
distance.}
...
\marginpar{See \texttt{COMP34411/programs/page-rank} in my third year notes Git
repo for an implementation of this.}
...
\begin{description}
  ...
  \item \textbf{Halfway-decent}\\
    \marginpar{\textit{Subsumption, noun:} incorporating something under a more 
    general category}
    Sentence $S$ entails $S'$ if every word in $S'$ is subsumed by a word in
...
\marginpar{This is dynamic programming, yay!}
...
\marginpar{I implemented dynamic time warping to understand it better; take a
look in the notes git repo under \texttt{COMP34411/programs}. You can use it to
check you get runs of DTM correct (I think it works, but haven't tested it
extensively).}
...
\marginpar{Through I've never tried this, I view this use of exceptions as 
badly thought out at the least, but sacrilegious, irreverent and heretic at 
worst. I don't want to descend into the static and dynamic typing debate, but
maybe an \textit{Either} type might work better?}
...
If you're trying to understand sound from a computer's point of view, all you
...
\marginpar{There's more on FFT in my \texttt{COMP28512} notes I think.}
...
So according to Figure~\ref{fig:gym-model}, if you see me warming up in the gym,
...
\marginpar{This is a hidden markov model now, as opposed to a markov model.}
...
The Viterbi Algorithm tries to find the most likely route through a HMM given
...
\subsubsection{Features of sounds}
...
\marginpar{Maybe consult the \texttt{COMP28512} notes if you're struggling to 
understand some terminology here. Unfortunately, I don't have time to provide a 
...
A solution is to write a grammar to define what we might be said. This does a
...
also lent me her nodes for this bit as well!
...
\marginpar{See the code for the gory details.}

Here I run page rank on a graph consisting of different interpretations of
