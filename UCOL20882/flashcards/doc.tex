...
\flashcard{Embryonic stem cells are \blank{immportal} and \blank{pluripotent}.}
...
\card{Describe the plasticitity of ASC's}{Most adult stem cells can trans-differentiate in the lab, but this is a low efficiency process.}
...
\card{What is GVHD?}{Graft Vs Host Disease, where the immune cells in the transpant attack the host.}

\card{Why are neonatal (UC cells) less immunogenic?}{Embroys and foetuses have to evade the mother's immune system, so there are less surface markers on cells. Also, newly born babies have no/little immune system so there is less chance of GVHD.}
...
\card{What enzyme do most cancer cells produce and why?}{Telomerase so that the cells are immortal and divide indefinately.}

\card{What is a bank of ESC lines?}{A bank of embronic stem cells, where each `line' of cells is derived from a single embryo.}

\card{What are the three sources of human stem cells?}{Embronic SC's, Neonatal SC's, adult SC's (bone marrow, fat tissue (liposuction), skin).}

\card{How could we make a stem cell with only some skin cells?}{Make it into an induced pluripotent stem cell in the lab.}

\card{What are the currently approved stem cell based therapies?}{Skin grafts, Hematopoietic SC transpant from adult bone marrow or neonatal cells.}

\card{How does a bone marrow transpant to cure leukemia work?}{1. Get a matching donor 2. Replicate stem cells ex vitro 3. Destroy bone marrow in patient using irrediation and chemotherapy 4. transplant stem cells into patient.}
...
\flashcard{Clinical trials take \blank{a long time}, and \blank{less than 10} therapies are in phase 3 for stem cell treatements. Foreign clinics advertise MSC treatements, but none have published data from clinical trials.}

\flashcard{Most trials for stem cell therapies are carried out with MSC's (\blank{70 percent}), HSC's count for \blank{20 percent}. ESC's are around \blank{2 percent} and are being tested with \blank{eyes} since they are \blank{immuno-priviledged}.}

\flashcard{SC's can be used for \blank{replacing cells (e.g. transplants)}, \blank{repairing cells (e.g. genetically modify SC's outide the body and re-implant)} and \blank{protecting via MSC immunosupression}.}

\card{For repairing and replacing cells, what type of cell should we use?}{The patients own cells (autologous transplants). This requires adult stem cells that are reasoably plastic though, and its hard to isolate ASC's in the lab. Otherwise, use donor SC's with low immunogenicity.}
...
\card{How to do parkinsons in a dish?}{
  \begin{tabular}{rl}
    -&Collect skin cells\\
    ...
    -&Observe parkinsons-like features
  \end{tabular}
}
...
-&Predict suseptability of developing a disease\\
...
\card{What genes increase your risk of breast and overian cancer and how much by?}{BRAC1, BRAC2; 85 percent higher lifetime chance of breast cancer and 60 percent chance of ovarian cancer.}
...
\flashcard{It's easy to take biopsy of cancer tumors (because they're by definition, not needed), so they can have their genome sequenced to see what genes the cancers have.}

\flashcard{There are drugs (Ivacaftor) that target the \blank{gene underlying cause} of diseases rather than just treating symtoms.}
