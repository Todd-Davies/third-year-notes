...
\card{What will these mutations result in when the gamete is combined
  with
  another?\\\includegraphics[scale=0.4]{images/meiosis-mutations}}{Left
...
\flashcard{\footnotesize The embryo starts with a zygote (\blank{totipotent}.\\ It
...
\card{Why are neonatal (UC cells) less immunogenic?}{Embryos and
  fetuses have to evade the mother's immune system, so there are less
...
\flashcard{SC's can be used for \blank{replacing cells
    (e.g. transplants)}, \blank{repairing cells}
  (e.g. \blank{genetically modify SC's outside the body and
    re-implant}) and \blank{protecting via MSC immunosupression}.}
...
\card{Give examples of ecosystem services.}{Protection of water
  resources, controbution to climate stability, maintenance of
  ecosystems, pollution breakdown and absorption, nutrient storage and
  recycling, soil formation etc...}

\card{Biological resources include...}{Food, medical resources and
...
\card{Define genetic drift}{When a population has little genetic
  variation, any disease that all individuals are susceptible to could
...
\flashcard{Greater Prairie Chicken were fragmented by
...
\card{Define ecosystem diversity}{Encompasses the variety of habitats
  that occur in a region, or the mosaic of patches found within a
  landscape}

\flashcard{An ecosystem can \blank{cover a large area} such as a whole
  forest or a \blank{small area} such as a pond.}

\card{Give four causes of biodiversity loss}{Habitat loss, introduced
  species, over exploitation, pollution.}

\flashcard{Most threatened species are imperilled \blank{for more than
    one reason}.}
...
\card{What is a keystone species?}{A keystone species is a species
  who's very presence contributes to a diversity of life and WHO's
  extinction would lead to the extinction of many others. They help
...
\flashcard{The Isthmohyla rivularis frog was thought to be extinct in
...
\card{Define an alien species}{Alien species are by definition taxa
  ...
  intentionally or unintentionally by human agency.}

\card{Define native}{An organism that is living in its home environment.}

\card{Define endemic}{An organism that is only found in/confined to a
  particular location}

\card{Define non-native species (NNS)}{To any species or race that
  does not occur naturally in an area.}

\card{Define Naturalised}{A non-native that has become a part of its
  new environment}

\card{Define invasive}{A non-native that has spread to become a
  dominant member of its new environment.}

\card{Define Invasive non-native}{A  species that becomes so abundant
  after arrival that it damages biodiversity and often causes
  substantial economic or health problems is (INNS)}

\card{What are the three stages in invasion biology
  theory?}{Introduction, colonisation and establishment, invasive
  spread (only now is a species invasive).}

\card{What makes an invasive species successful}{They are
  r-strategists (grow quickly, produce many offspring, short
  generation time), good dispersion, generalists (highly adaptable,
  broad geographic range, broad diet)}

\card{What are the effects of invasive non-native species}{They may
  invade adjacent geographic areas, displace native organisms, or
  produce hybrids with native species (an irreversible change in the
  genetic pool). They also change the food web and can cause
  hyperpredation.}

\card{Remember, not all invasive species are non-native. E.g. Bracken
  can invade heathland.}{Remember it.}

\flashcard{Japanese knotweed was introduced from \blank{Japan} in the
  \blank{1800's} as an ornamental plant and cannon fodder. It has a
  dense leaf canopy, and is tolerant to \blank{soil acidity},
  \blank{heavy metal contamination} and \blank{pollution}, growing at
  around one meter per month. It is hard to eradicate and small
  fragments (as small as \blank{0.5 grams}) grams can grow new
  plants. Both \blank{chemical} and \blank{biological} control methods
  are used (\blank{chemical} is best).}

\card{What is being done to tackle non-native invasive
  species?}{Prevention (risk assessments), early detection (monitoring
  and rapid response), mitigation + eradication (kill the invaders),
...
\card{\includegraphics[width=\cardwidth]{images/brain-un}}{\includegraphics[width=\cardwidth]{images/brain}}
...
\flashcard{The frontal lobe has the \blank{primary motor cortex} and
  the \blank{central sulcus}. The latter separates the frontal lobe
...
\flashcard{The brainstem has \blank{cardiovascular and respiratory
    centres} are here. This dictates things like \blank{heart beat
...
\card{What makes up the Central Nervous System (CNS) )}{The brain,
...
\card{\includegraphics[height=\cardheight]{images/neuron-un}}{\includegraphics[height=\cardheight]{images/neuron}}
...
\flashcard{The gaps between myelin sheathes are called \blank{Nodes of Ranvier}}
...
\card{\includegraphics[height=\cardheight]{images/ac-pot-bl}}{\includegraphics[height=\cardheight]{images/ac-pot}}
...
\card{\includegraphics[width=\cardwidth]{images/synapse-bl}}{\includegraphics[width=\cardwidth]{images/synapse}}
...
\card{What are the three types of seizure?}{Generalised (all over the
...
\flashcard{Jacksonian seizure is in the \blank{motor cortex}
  (\blank{frontal lobe}), \blank{twitching} progresses from \blank{one
    part of the body} to \blank{another, stopping in-between}}.

\flashcard{Todd's paralysis is \blank{limpness} after a seizure.}

\flashcard{Causes of seizures include \blank{trauma to the brain} such
...
\card{How is a seizure diagnosed and treated?}{Diagnoses wise,
  ...
  is taken. Treatment wise, patients should not be restrained since
  they could hurt themselves/others, and medication such as diazepam
  could be taken.}

\card{What is diazepam}{A muscle suppressant given to fitting
  people. Too much can inhibit breathing.}

\flashcard{Epileptic people must not have a fit for \blank{ten years}
...
\flashcard{Diagnosis for parkinson's is \blank{clinical}, and patients
  ...
  usually caused by \blank{bhronchopneumonia}. There are treatments
...
\card{List the effects of morphine}{Pain relief, euphoria, itching
  ...
  be remedied with other drugs).}

\card{How does morphine work?}{It binds to receptors on neurones, and
...
\flashcard{Emerging and re-emerging infectious diseases today include
  ...
  unrecognised infections appearing in \blank{areas with habitat
    ...
    to treatment (drug resistance)} and diseases thought \blank{to
...
\flashcard{\blank{Hospital acquired infections} (HAI's) are a big issue
...
\card{Where does influenza infect in the mammal?}{The respiratory
...
\card{Why are young healthy people vulnerable to flu?}{Since they have
...
\card{Symptoms of ebola?}{Asymptomatic for 2-21 days. and not
  ...
  vomiting, headache etc). After that, haemorrhagic manifestations
...
\flashcard{Ebola infects \blank{immune system} cells, which then
  ...
  immune cells working, which means that cells respond to the cytokines
...
\card{What diagnostic tests are there for ebola?}{Can't detect
  ...
  antibody response is detected (and patients often die before
